    ...
    <div class="tabset-container" data-listelement-count="$Elements.Elements.Count">
        <% if $Elements %>
            ...
            <% loop $Elements.Elements %>
                <li class="nav-item">
                    <a class="nav-link <% if $First %>active<% end_if %>" id="tab-button-{$ID}" data-bs-toggle="tab" href="#tab-{$ID}" role="tab" aria-controls="tab-{$ID}" aria-selected="<% if $First %>true<% else %>false<% end_if %>"><span>$Title</span></a>
                </li>
            <% end_loop %>
            </ul>
